...
The `flokay:spec` skill drives a conversational, interactive requirement-discovery process that translates proposal capabilities into detailed behavioral specifications. It elicits edge cases, error conditions, and scenarios through guided questions, producing spec files in WHEN/THEN scenario format. It also supports deferred-to-design markers for requirements that depend on unresolved architectural decisions.
